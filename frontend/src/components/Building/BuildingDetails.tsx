    ...
    <SideNavbar/>
      ...
      {!showFloors ? (
        <form className="text-center" onSubmit={(e) => e.preventDefault()}>
          <div className="bg-white p-16 rounded-sm shadow-amber-800 shadow-2xl">
            <div>
              ...
              <input
                ...
                className="block outline-none w-full mt-2 p-3 rounded-md bg-primary-bg
                 text-stone-900 font-semibold"
              />
            </div>
            <button
              onClick={createBuildingAndFloors}
              className="relative top-7 left-[105px] 
              px-5 py-3 bg-amber-900 text-white rounded-md hover:bg-amber-950"
            >
              Save
            </button>
          </div>
